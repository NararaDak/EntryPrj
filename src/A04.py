    ...
    def _visualize_results(self, epoch, max_epochs, predictions, mode="eval", test_img_dir=None):
        ...
        if mode == "eval":
            ...
            if isinstance(predictions, dict):
                ...
                metrics_text += f"mAP50: {f'{mAP50_val:.4f}' if mAP50_val is not None else 'N/A'}\n"
                metrics_text += f"mAP50-95: {f'{mAP50_95_val:.4f}' if mAP50_95_val is not None else 'N/A'}\n"
                metrics_text += f"Precision: {f'{precision_val:.4f}' if precision_val is not None else 'N/A'}\n"
                metrics_text += f"Recall: {f'{recall_val:.4f}' if recall_val is not None else 'N/A'}"
            else:
