...
def display_summary():
    """PDF 리포트를 표시하는 함수"""
    file = r"D:\01.project\EntryPrj\data\report.pdf"
...
def display_model_study():
    st.subheader("모델 연구")
    st.write("이 탭에서는 다양한 AI 모델 구조, 하이퍼파라미터, 실험 결과 등을 정리하고 비교합니다.")
    st.markdown("""
    - **YOLOv8**: 최신 Object Detection 모델, 빠른 추론과 높은 정확도
    - **Faster R-CNN**: 고전적인 2단계 검출기, 높은 정밀도
    - **실험 결과**: 각 모델별 mAP, F1-score, 학습/추론 속도 등 비교
    """)
    st.info("모델별 상세 실험 결과와 그래프, 코드 스니펫 등을 추가하세요.")